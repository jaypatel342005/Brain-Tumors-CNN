...
// Map prediction IDs to distinct styles and text
...
export default function Home() {
  ...
  const analyzeScan = async () => {
    ...
    try {
      ...
      setTimeout(() => {
        setResult({ success: false, prediction: "", class_idx: -1, error: "Failed to connect to the backend analysis engine." });
        setIsLoading(false);
      }, 1000);
    } 
  };

  const activeResult = result?.success && result?.prediction ? predictionDetails[result.prediction] : null;
